...
cls_wt <- list(
    "0" = as.numeric((1 / tbl["0"]) * (length(tr$y) / 2)), 
    "1" = as.numeric((1 / tbl["1"]) * (length(tr$y) / 2))
)
...
# 7. Predict 預測 ------------------------------------------------------------------
################################################################################
pred_lstm <- predict(final_mod, te$X)
pred_lstm_label <- ifelse(pred_lstm > 0.5, 1, 0)
mean(pred_lstm_label == te$y)

# confusion matrix
library(dplyr)
df <- data.frame(
    predicted = pred_lstm_label,
    actual = te$y
)
